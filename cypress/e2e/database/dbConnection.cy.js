...
describe('DB Connection', function() {
    ...
    it('Test Case 1 - Create user and validate in database', function() {
        ...
        cy.task('runQuery', 'SELECT * FROM student WHERE email = \'azizelm@techglobal.com\'').then((rows) => {
     ...
     */
        ...
        it('Test Case 2 - Update user and validate in database', function() {
           ...
           backendPage.getUserEditButton()
           backendPage.getModalEmail().clear().type('aziz.elm@techglobal.com')
           backendPage.getModalUpdateButton().click()
           backendPage.getEditModal().should('not.exist')
    
            cy.task('runQuery', 'SELECT * FROM student WHERE email = \'aziz.elm@techglobal.com\'').then((rows) => {
